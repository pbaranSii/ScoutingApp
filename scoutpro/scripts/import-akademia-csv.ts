...
const DEFAULT_FILE = "C:\\Projekty\\CursorAplications\\ScoutApp Materiały\\Dane KS Polonia\\Import akademia.csv";
...
function buildCanonicalRows(rawRows: RawRow[]): { rows: CanonicalRow[]; allHeaders: string[] } {
  ...
  for (let i = 0; i < rawRows.length; i += 1) {
    ...
    const potentialNow = parseScore(normalized.performance);
    // Header "Potencjał przyszły" may normalize to either:
    // - "potencjalprzyszly" (full transliteration)
    // - "potencjaprzyszy" (when "ł" is stripped)
    // Read both aliases to avoid losing values.
...
async function main() {
  ...
  if (!["AKADEMIA", "SENIOR"].includes(targetArea)) {
    throw new Error("Invalid --area. Allowed: AKADEMIA|SENIOR.");
  }
  if (!placeholderScoutEmail) {
    throw new Error("Missing --placeholder-scout-email or PLACEHOLDER_SCOUT_EMAIL.");
  }
  ...
  const report: ImportReport = {
    ...
    entities: { clubsInserted: 0, playersInserted: 0, observationsInserted: 0, scoutsMapped: 0, scoutsPlaceholder: 0 },
  ...
  if (!firstSheetName) throw new Error("CSV/XLSX has no sheets.");
  ...
  if (!placeholderScout) {
    throw new Error(`Placeholder scout user not found by email: ${placeholderScoutEmail}`);
  }
  ...
  const observationsToInsert: ObservationsInsert[] = [];
  ...
  for (const row of validRows) {
    ...
    observationsToInsert.push({
      ...
      source: mapSourceFromRodzaj(row.rodzaj),
      observation_date: row.observationDate ?? new Date().toISOString().slice(0, 10),
  ...
  if (!dryRun && observationsToInsert.length > 0) {
    const chunkSize = 500;
  ...
  report.entities.observationsInserted = observationsToInsert.length;

  report.finishedAt = new Date().toISOString();
  await fs.mkdir(REPORTS_DIR, { recursive: true });
  const reportPath = path.join(REPORTS_DIR, `import-akademia-${nowIsoCompact()}.json`);
