-- Hotfix for oilillvaatchsyvqbyxo: clubs.area is TEXT, not public.area_type.
-- Recreate publish function with TEXT-safe clubs upsert.
...
begin
  ...
  -- Clubs: TEXT-safe (target DB has clubs.area as text).
  ...
  -- Players: dedupe by (first_name,last_name,birth_year,club_name).
  for p in
    select value as row
