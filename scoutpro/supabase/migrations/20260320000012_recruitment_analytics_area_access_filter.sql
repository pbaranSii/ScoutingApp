-- Enforce area-based visibility for `Recruitment Analytics` RPCs.
-- Problem: analytics RPC functions were not explicitly filtering by `current_area_access()`
-- when aggregating across `players` / `pipeline_history`, so Trener/Dyrektor could see cross-area results.
--
-- Fix: re-define analytics RPC functions used by the UI and filter cohort candidates by:
--   - players.age_category_id -> categories.area
--   - categories.area must match current_area_access(), unless current_area_access() = 'ALL'
...
begin
  ...
  cohort_players as (
    ...
    where
      ...
      and (ri.v is null or cardinality(ri.v) = 0 or coalesce(p.region_id, cl.region_id) = any(ri.v))
      and (ci.v is null or cardinality(ci.v) = 0 or p.club_id = any(ci.v))
      and (bys.v is null or cardinality(bys.v) = 0 or p.birth_year = any(bys.v))
      and (pos.v is null or cardinality(pos.v) = 0 or p.primary_position = any(pos.v))
  ),
